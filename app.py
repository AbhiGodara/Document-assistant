...
def sidebar_profiles():
    ...
    st.sidebar.markdown("### 🎉Author: Abhishek Godara")
    st.sidebar.markdown("### 🔗 Connect With Me")
    st.sidebar.markdown("""
    <hr>
    <div class="profile-links">
        <a href="https://github.com/AbhiGodara" target="_blank">GitHub
        </a><br><br>
        <a href="https://www.kaggle.com/abhishekgodara" target="_blank">Kaggle
        </a><br><br>
        <a href="mailto:abhishekgodara032@gmail.com">Email
        </a><br><br>
        <a href="https://www.linkedin.com/in/abhishek-godara-a82a1b2a0/" target="_blank">LinkedIn
        </a>
    </div>
    <hr>
...
def translate_full_document(text, target_language):
    try:
        with st.spinner("Translating full document... This may take some time."):
        ...
        st.error(f"Full document translation failed: {str(e)}")
        return None
...
with st.sidebar:    
    ...
    if st.session_state.last_query_time:
         ...
         5. Download full translated document
         """)
...
# Add footer
st.divider()
st.subheader("📥 Download Full Translated Document")
if "full_text" in st.session_state and st.session_state["full_text"]:
    if st.button("Translate & Prepare Download"):
        translated_doc = translate_full_document(
            st.session_state["full_text"],
            lang_code
        )
        if translated_doc:
            st.session_state["translated_document"] = translated_doc
            st.success("Translation completed! Ready for download.")

    if "translated_document" in st.session_state:
        st.download_button(
            label="⬇️ Download Translated File (.txt)",
            data=st.session_state["translated_document"],
            file_name=f"translated_document_{language}.txt",
            mime="text/plain"
        )
else:
    st.info("Upload a document to enable full document translation.")
